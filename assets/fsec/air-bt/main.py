...
async def scan_phase(scanner: BLEScanner, args) -> list[BTDevice]:
    ...
    # Reset any terminal state left by Live (scroll region, colors, cursor attrs)
...
def show_target_table(devices: list[BTDevice]):
    _fc = {"OPEN_WRITE": "bright_red", "OPEN_READ": "red", "NO_BONDING": "yellow",
           ...
           "Chinese OEM": "cyan"}
    console.print(f"\n[bold cyan]Discovered Devices[/] — [white]{len(devices)} found[/]\n")
    for i, dev in enumerate(devices, 1):
        sc     = _SEC_COLOR.get(dev.sec_score, "white")
        flags  = " ".join(f"[{_fc.get(f,'white')}]{f}[/]" for f in dev.sec_flags[:3]) or "[dim]-[/]"
        cve    = (f"  [{_TYPE_COLOR.get(dev.known_vuln_type or '','red')}]{dev.known_vuln}[/]"
                  if dev.known_vuln else "")
        console.print(
            f"  [bold bright_yellow]{i:>2}.[/]  "
            f"[cyan]{dev.mac}[/]  "
            f"[bright_white]{(dev.name or '?')[:18]:<18}[/]  "
            f"{_rssi_bar(dev.rssi)} {dev.rssi:>4}  "
            f"[{sc}]{dev.sec_score}[/]  "
            f"{flags}{cve}"
        )
    console.print()
...
def show_attack_menu(dev: BTDevice, menu: list[dict]):
    ...
    _RED = {"exploit", "elk_rainbow", "poc_hid", "poc_write_probe", "whisperpair",
            "dfu_probe", "tuya_control", "govee_control", "sweyntooth", "hearing_aid",
            "blueborne_info", "nordic_uart", "speaker_control", "smart_lock",
            "ibeacon_track", "overflow_probe", "mutfuzz", "reconnect_bypass"}
    _TAG_COLOR = {
        "gatt": "green", "fuzz": "red", "cve_info": "red",
        "notify": "cyan", "poc_audio": "cyan", "poc_sensor": "cyan",
        "poc_fitness": "cyan", "poc_health": "cyan", "poc_plug": "cyan",
        "poc_generic": "cyan", "auto_poc": "cyan", "mibeacon": "cyan",
        "payload": "yellow", "back": "dim", "details": "white", "export": "white",
    }

    console.print()
    for i, item in enumerate(menu):
        tag   = item["tag"]
        color = "bright_red" if tag in _RED else _TAG_COLOR.get(tag, "white")
        desc  = item.get("desc", "")
        console.print(f"  [bold bright_yellow]{i:>3}.[/]  [{color}]{item['label']}[/]")
        if desc:
            console.print(f"       [dim]{desc[:70]}[/]")
    console.print()
